...
desc: >
  etcd corrupted, no backup, source data gone? Here's how we salvaged a Milvus
  collection from MinIO binlogs with bulkInsert, and how to validate segments
  before importing.
...
<pre><code translate="no">Important: This <span class="hljs-keyword">is</span> <span class="hljs-keyword">not</span> a standard backup <span class="hljs-keyword">or</span> recovery procedure. It <span class="hljs-keyword">is</span> a last-resort salvage path <span class="hljs-keyword">for</span> cases where etcd <span class="hljs-keyword">is</span> unavailable, the original data <span class="hljs-keyword">is</span> gone, <span class="hljs-keyword">and</span> no Milvus Backup exists.MinIO <span class="hljs-keyword">is</span> <span class="hljs-keyword">not</span> the source of truth <span class="hljs-keyword">for</span> Milvus metadata. It can show that <span class="hljs-built_in">object</span> files exist, but it cannot reliably tell you which segments are still live. A recovery process must therefore validate candidate segments rather than importing every readable directory, <span class="hljs-keyword">in</span> particular to avoid re-importing historical <span class="hljs-keyword">or</span> pre-compaction segments <span class="hljs-keyword">as</span> duplicates.
